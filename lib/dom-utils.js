...
export function getIdOfNextSvgElement(currentElement, direction = 'forwards',
  elementNames = navigationElementsSelector,
  incr = 'all') {
    ...
    if (found &&
      ...
      if (incr == 'measure') {
        ...
        if (direction == 'backwards') { // to find first element of measure
          console.info('backwards in measure: ',
            this.getFirstInMeasure(currentMeasure, elementNames, staffNo, layerNo));
...
export function isFirstElement(elementId) {
  ...
  console.info('isFirstElement staffNo: ', staffNo);
  console.info('isFirstElement layerNo: ', layerNo);
