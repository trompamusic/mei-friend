  ...
  */
  ...
  delete(textEditor, id) {
    ...
    if (att.modelControlEvents.concat(['accid', 'artic', 'clef', 'octave'])
      ...
      let first = true;
      for (ch of element.childNodes) {
        if (ch.nodeType === Node.TEXT_NODE) continue;
        if (first) {
          speed.replaceInTextEditor(textEditor, element, false, ch);
          first = false;
        } else {
          textEditor.insertNewline();
          textEditor.insertText(speed.xmlToString(ch));
        }
        element.parentNode.insertBefore(ch, element);
        console.info('xml:id: ' + ch.getAttribute('xml:id'));
        selectedElements.push(ch.getAttribute('xml:id'))
      }
  ...
  addBeamElement(textEditor, elementName = 'beam') {
    ...
    let buffer = textEditor.getBuffer();
    this.selectedElements = speed.filterElements(this.selectedElements, this.xmlDoc);
    // console.info('addBeamElement(' + elementName +  //   '): selectedElements:', this.selectedElements);
    ...
    if (par1.getAttribute('xml:id') == n2.parentNode.getAttribute('xml:id')) {
      ...
      for (n of nodeList) {
        ...
        if (n == n2) {
          beam.appendChild(n);
          let range = speed.replaceInTextEditor(textEditor, n, true, beam);
          if (range)
