...
export default class MeiFriendView {
  ...
  constructor(vt) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      this.lastNoteId = utils.getIdOfItemAtCursor(item.getBuffer(), item.getCursorBufferPosition());
      ...
      this.waitForElement(`#${this.breaksSelector.id}`, () => {
        $(`#${this.breaksSelector.id}`).off('change').on('change', () => {
          ...
          this.vodMode = (this.vrvOptions.breaks == 'line' || this.vrvOptions.breaks == 'encoded') ? true : false;
  ...
  loadXml(textEditor) {
    if (this.encodingHasChanged) { // update only if encoding has been edited or updated
      this.xmlDoc = this.parser.parseFromString(textEditor.getText(), "text/xml");
      breakList = this.xmlDoc.querySelectorAll("sb, pb");
      this.pageCount = breakList.length + 1;
      if (breakList.item(0).nodeName == 'sb' || breakList.item(0).nodeName == 'pb') this.pageCount--;
      console.info('loadXml: pageCount: ' + this.pageCount);
      this.encodingHasChanged = false;
    }
    mei = vod.getVodPage(this.xmlDoc, this.currentPage);
    // console.info('loadXml mei: ', mei);
    return mei;
    // console.info('vodNode.innerHTML:\n', mei);
  }
  ...
  showCurrentPage(page = this.currentPage) {
    ...
    try {
      this.verovioPanel.innerHTML = this.vrvToolkit.renderToSVG(page, {});
    } catch (e) {
      console.log(e);
      return;
    }
  ...
  handleClickOnNotation(e, textEditor) {
    ...
    if (((navigator.appVersion.indexOf("Mac") !== -1) && e.metaKey) || e.ctrlKey) {
      ...
      let range = utils.locateIdInBuffer(textEditor.getBuffer(), itemId);
      if (range) {
        textEditor.setCursorBufferPosition([range.start.row, range.start.column]);
      }
      this.selectedElements = [];
  ...
  updateHighlight(textEditor) {
    ...
    if (this.selectedElements.length > 1) {
      ...
      id = utils.getIdOfItemAtCursor(textEditor.getBuffer(), textEditor.getCursorBufferPosition());
  ...
  */
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '', formString = '', useTstamps = false) {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      let buffer = textEditor.getBuffer();
      uuid = elementName + '-' + utils.generateUUID();
  ...
  invertPlacement(textEditor, modifier = false) {
    ...
    let elementName = '';
    for (id of this.selectedElements) {
      let chord = utils.insideChord(id);
      if (chord) id = chord;
      ...
      if (false && att.attPlacement.includes(elementName) && modifier) { // CTRL + X @place="between/within"
        ...
        attribute = 'place';
        if (attr.hasOwnProperty(attribute) && (att.dataPlacement.includes(value) && attr.place != 'between')) {
          value = 'between';
        }
        ...
        attribute = 'place';
        if (attr.hasOwnProperty(attribute) && (att.dataPlacement.includes(value) && attr.place != 'below')) {
          value = 'below';
        }
        this.executeSetAction(id, attribute, value);
  ...
  updateNotationToTextposition(textEditor, update = true) {
    var page;
    if (this.vodMode) {
      page = vod.getPageNumberAtCursor(textEditor);
    } else {
      let id = utils.getIdOfItemAtCursor(textEditor.getBuffer(), textEditor.getCursorBufferPosition());
