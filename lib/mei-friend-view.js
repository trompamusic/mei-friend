  ...
  */
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '', formString = '', useTstamps = false) {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      // use Verovio insert action for all control elements but arpeg
      if (elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // || elementName == 'dir' || elementName == 'dynam') {
        ...
        if (endId) { // check elementName for required and optional endId!!
          editorAction.param.endid = endId;
        }
        // add text
        ...
        if (placementString) {
          if (attCurvature.includes(elementName)) {
            this.executeSetAction(uuid, 'curvedir', placementString);
          } else if (attPlacement.includes(elementName)) {
            this.executeSetAction(uuid, 'place', placementString);
          }
        }
  ...
  invertPlacement(textEditor, modifier = false) {
    ...
    for (id of this.selectedElements) {
      ...
      let attribute = '';
      let value = 'above';
      if (attPlacement.includes(elementName) && modifier) { // CTRL + X @place="between/within"
    ...
    textEditor.setText(this.vrvToolkit.getMEI());
    range = locateIdInBuffer(buffer, id);
    if (range) textEditor.setCursorBufferPosition([range.start.row, range.start.column]);
    this.selectedElements = [];
