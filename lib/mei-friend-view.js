...
export default class MeiFriendView {
  ...
  showCurrentPage(page = this.currentPage) {
    console.info('showCurrentPage(): ' + page + ', pgCnt: ' + this.pageCount);
  ...
  setCursorToPageBeginning(txtEdr) {
    ...
    let range = utils.locateIdInBuffer(txtEdr.getBuffer(), id);
    if (range) {
  ...
  updateHighlight(txtEdr) {
    ...
    if (this.selectedElements.length >= 1) {
      ...
      id = utils.getElementIdAtCursor(txtEdr.getBuffer(),
  ...
  / by `direction`
  */
  navigate(txtEdr, elementName = 'note', direction = 'forwards') {
    if (this.lastNoteId == '') {
      this.setCursorToPageBeginning(txtEdr);
    }
  ...
  addControlElement(txtEdr, elementName = 'slur', placementString = '',
    ...
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      if (false) { //elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // || elementName == 'dir' || elementName == 'dynam') {
        ...
        txtEdr.setText(this.vrvToolkit.getMEI());
  ...
  addBeamElement(txtEdr, elementName = 'beam') {
    ...
    if (par1.getAttribute('xml:id') == n2.parentNode.getAttribute('xml:id')) {
      ...
      this.selectedElements = [];
      this.selectedElements.push(uuid);
      this.speedMode ? this.updateLayout(txtEdr) : this.updateAll(txtEdr);
      this.setFocusToVerovioPane();
    } else {
  ...
  invertPlacement(txtEdr, modifier = false) {
    if (true || this.speedMode) {
      this.invertPlacementDom(txtEdr, modifier);
      return;
    }
  ...
  invertPlacementDom(txtEdr, modifier = false) {
    this.loadXml(txtEdr);
    let buffer = txtEdr.getBuffer();
    let ids = utils.sortElementsByScoreTime(this.selectedElements, this.vrvToolkit);
