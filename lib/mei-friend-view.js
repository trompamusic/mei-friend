  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    if (!measure) {
      ...
      if (elementName == 'note') {
        id = utils.getIdOfNextSvgElement(element, direction, elementsList);
        if (!id && direction == 'forwards') {
          ...
          if (st) {
            let l = st[st.length - 1].querySelectorAll('.layer[data-n="' + layerN + '"]');
            if (l) {
              let e = l[l.length - 1].querySelectorAll(elementsList);
              if (e) id = e[e.length - 1].getAttribute('id');
              else id = st.querySelector(elementsList).getAttribute('id');
            }
          }
        }
