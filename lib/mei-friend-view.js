  ...
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    if (!measure) {
      ...
      if (elementName == 'note' || elementName == 'measure') {
        ...
        if (ch) {
          ...
          ch.querySelectorAll('.note').forEach(item => {
            let newDiff = Math.abs(domutils.getY(item) - y);
      ...
      if (elementName == 'layer') {
        ...
        let els = Array.from(measure.querySelectorAll(elementsList));
        els.sort(function(a, b) {
          if (Math.abs(domutils.getX(a) - x) > Math.abs(domutils.getX(b) - x))
            return 1;
          if (Math.abs(domutils.getX(a) - x) < Math.abs(domutils.getX(b) - x))
            return -1;
          if (domutils.getY(a) < domutils.getY(b))
            return (direction == 'upwards') ? 1 : -1;
          if (domutils.getY(a) > domutils.getY(b))
            return (direction == 'upwards') ? -1 : 1;
          return 0;
        });
