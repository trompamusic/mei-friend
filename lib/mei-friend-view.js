...
export default class MeiFriendView {
  ...
  */
  ...
  delete(textEditor, id) {
    ...
    if (att.modelControlEvents.concat(['accid', 'artic', 'clef'])
  ...
  addOctaveElement(textEditor, place = 'above', distance = '8') {
    this.loadXml(textEditor);
    if (this.selectedElements.length < 1) return;
    let buffer = textEditor.getBuffer();
    ...
    console.info("Staff of n1: ", st1);
    if (!st1) return;
    let staffNumber = st1.getAttribute('n');
    console.info("Staffnumber of n1: ", staffNumber);
    let plist = []; // array of octave ids
    let allStaves = this.xmlDoc.querySelectorAll("staff[n='" + staffNumber + "']");
    // console.info('allStaves: ', allStaves);
      ...
      for (st of allStaves) {
        ...
        if (staffFound) {
          ...
          for (n of notes) {
            ...
            if (noteFound) plist.push(n.getAttribute('xml:id'));
            if (n.getAttribute('xml:id') == id2) {
              staffFound = false;
              break staffLoop;
            }
          }
          console.info('Staff: ', st)
        }
      }
    this.selectedElements = plist;
    this.updateLayout(textEditor);
    this.setFocusToVerovioPane();
    // let n1 = this.xmlDoc.querySelector("[*|id='" + id1 + "']");
    // let n2 = this.xmlDoc.querySelector("[*|id='" + id2 + "']");
