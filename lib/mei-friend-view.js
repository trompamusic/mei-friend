...
export default class MeiFriendView {
  ...
  setCursorToPageBeginning(textEditor) {
    ...
    let stNo, lyNo;
    let rg = utils.locateIdInBuffer(buffer, id);
    let element = document.querySelector('g#' + id);
    if (!element) { // if element not on page, take first on page matching st/ly
      console.info('setCursorToPgBeg off-screen. id: ' + id + ', range: ', rg);
      stNo = utils.getElementAttributeAbove(buffer, rg.start.row, 'staff');
      lyNo = utils.getElementAttributeAbove(buffer, rg.start.row, 'layer');
