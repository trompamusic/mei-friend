...
export default class MeiFriendView {
  ...
  loadXml(textEditor) {
    if (this.encodingHasChanged) { // update only if encoding has been edited or updated
      ...
      elements = this.xmlDoc.querySelectorAll("measure, sb, pb");
      console.info('loadXml: breaks and measures: ', elements);
      // count pages
      this.pageCount = (['sb', 'pb'].includes(elements.item(0).nodeName)) ? 0 : 1; // skip trailing break
      for (e of elements) {
        if (['sb', 'pb'].includes(e.nodeName)) this.pageCount++;
      }
      console.info('loadXml: pageCount: ' + this.pageCount);
      this.encodingHasChanged = false;
    }
    mei = vod.getVodPage(this.xmlDoc, elements, this.currentPage);
