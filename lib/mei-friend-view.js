...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      if (this.lastNoteId == null) {
        ...
        this.lastNoteId = document.querySelector('.note').getAttribute('id');
      }
  ...
  updateAll(textEditor, options = {}, setCursorToPageBeginning = false) {
    ...
    let element = document.querySelector('g#' + this.lastNoteId);
    let staffNo = element.closest('.staff').getAttribute('data-n');
    let layerNo = element.closest('.layer').getAttribute('data-n');
    if (this.speedMode) {
    ...
    if (setCursorToPageBeginning)
      this.setCursorToPageBeginning(textEditor, staffNo, layerNo);
  ...
  setVerovioOptions(newOptions = {}) {
    ...
    if (this.vrvOptions.breaks !== "none") {
      let width = Math.max(Math.round(dimensions.pageWidth * (100 / this.vrvOptions.scale)), 100);
      let height = Math.max(Math.round(dimensions.pageHeight * (100 / this.vrvOptions.scale)), 100);
  ...
  setCursorToPageBeginning(textEditor, staffNo, layerNo) {
    let id = '';
    let m = document.querySelector('.measure');
    console.info('m: ', m);
    console.info('staffNo: ', staffNo);
    console.info('layerNo: ', layerNo);
    if (m) id = domutils.getFirstInMeasure(m,
      domutils.navigationElementsSelector, staffNo, layerNo);
    console.info('id: ', id);
    if (!id) id = document.querySelector('.note').getAttribute('id');
    ...
    console.info('setCursorToPageBeginning(): lastNoteId: ' + this.lastNoteId +
      ', new id: ' + id);
  ...
  updateHighlight(textEditor) {
    ...
    if (this.selectedElements.length >= 1) {
      for (id of this.selectedElements) {
        ...
        // console.info('updateHighlight id: ', id);
      }
    } else {
  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    let layerNo = element.closest('.layer').getAttribute('data-n');
    if (!element) {
      id = this.setCursorToPageBeginning(textEditor, staffNo, layerNo);
      element = document.querySelector('g#' + id);
    }
    console.info('Navigate ' + direction + ' for: ', element);
    ...
    if (!measure) {
      ...
      let layerN = layer.getAttribute('data-n');
      let staffN = staff.getAttribute('data-n');
