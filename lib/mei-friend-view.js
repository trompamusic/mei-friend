...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      this.lastNoteId = utils.getElementIdAtCursor(item.getBuffer(), item.getCursorBufferPosition());
  ...
  setCursorToPageBeginning(textEditor) {
    ...
    let id = document.querySelector('.note').getAttribute('id');
  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    if (id == '')
      id = this.setCursorToPageBeginning(textEditor);
    ...
    if (!measure) {
      ...
      if (elementName == 'note' || elementName == 'measure') {
        id = utils.getIdOfNextSvgElement(
        ...
        if (!id && direction == 'forwards') {
          ...
          this.setFocusToVerovioPane();
          let st = document.querySelector('.staff[data-n="' + staffN + '"]');
          if (st) {
            let l = st.querySelector('.layer[data-n="' + layerN + '"]')
            if (l) id = l.querySelector(elementsList).getAttribute('id');
            else id = st.querySelector(elementsList).getAttribute('id');
          }
        } else if (!id && direction == 'backwards' && this.currentPage > 1) {
          this.updatePage(textEditor, 'backwards');
          this.setFocusToVerovioPane();
          let st = document.querySelectorAll('.staff[data-n="' + staffN + '"]');
          if (st) {
            let l = st[st.length - 1]
              .querySelectorAll('.layer[data-n="' + layerN + '"]');
            if (l) {
              let e = l[l.length - 1].querySelectorAll(elementsList);
              if (e && elementName == 'note')
                id = e[e.length - 1].getAttribute('id');
              else if (e && elementName == 'measure')
                id = e[0].getAttribute('id');
            } else {
              if (elementName == 'note')
                e = st.querySelectorAll(elementsList)
              if (e && elementName == 'note')
                id = e[e.length - 1].getAttribute('id');
              else if (e && elementName == 'measure')
                id = e[0].getAttribute('id');
