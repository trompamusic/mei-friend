...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      this.waitForElement(`#${this.breaksSelector.id}`, () => {
        $(`#${this.breaksSelector.id}`).off('change').on('change', () => {
          ...
          if (this.vrvOptions.breaks == 'line' ||
            ...
            this.vodCheckbox.disabled = false;
            this.vodMode = this.vodCheckbox.checked;
          } else {
            this.vodCheckbox.disabled = true;
            this.vodMode = false;
          }
          this.encodingHasChanged = true;
      ...
      this.waitForElement(`#${this.vodCheckbox.id}`, () => {
        $(`#${this.vodCheckbox.id}`).off('change').on('change', () => {
          this.vodMode = this.vodCheckbox.checked;
          this.encodingHasChanged = true;
          console.info('breaksSelector: vodMode: ' + this.vodMode);
          this.updateAll(item);
  ...
  */
  ...
  addControlElementDom(textEditor, elementName, placementString, formString, useTstamps) {
    ...
    let uuid = elementName + '-' + utils.generateUUID();
    newElement.setAttributeNS('http://www.w3.org/XML/1998/namespace', 'xml:id', uuid);
    ...
    this.xmlDoc.querySelector("[*|id='" + measureId + "']").appendChild(newElement); //.cloneNode(true));
  ...
  createHTMLElements(scale) {
    ...
    this.vodCheckbox = document.createElement('input');
    this.vodCheckbox.id = "vod-checkbox";
    this.vodCheckbox.setAttribute('type', 'checkbox');
    this.vodCheckbox.setAttribute('checked', 'false');
    this.vodCheckbox.checked = false;
    this.vodCheckbox.disabled = true;
    atom.tooltips.add(this.vodCheckbox, {
      title: 'Activate VOD mode (faster display)'
    });
    this.breaksCtrls.appendChild(this.vodCheckbox);
