...
export default class MeiFriendView {
  ...
  loadXml(textEditor) {
    let whichBreaks = ['sb', 'pb'];
    if (this.vrvOptions.breaks == "encoded") {
      whichBreaks = ['pb'];
    }
    console.info('whichBreaks: ', whichBreaks);
    ...
    if (this.encodingHasChanged) {
      ...
      for (e of elements) {
        ...
        if (countBreaks && whichBreaks.includes(e.nodeName)) this.pageCount++;
      }
    ...
    let mei = vod.getVodPageFromDom(this.xmlDoc, this.currentPage, whichBreaks);
