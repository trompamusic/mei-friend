  ...
  */
  ...
  delete(textEditor, id) {
    ...
    if (att.modelControlEvents.concat(['accid', 'artic', 'clef', 'octave'])
      ...
      // check @plist in octave to reset @oct and @oct.ges
      if (element.nodeName == 'octave') {
        let dis_place = element.getAttribute('dis.place');
        let dis = element.getAttribute('dis');
        let id1 = element.getAttribute('startid');
        let id2 = element.getAttribute('endid');
    ...
    if (par1.getAttribute('xml:id') == n2.parentNode.getAttribute('xml:id')) {
      ...
      for (n of nodeList) {
        ...
        if (n == n2) {
          ...
          textEditor.setCursorBufferPosition([range.start.row, range.start.column + 2]);
          break;
        }
        if (insert) {
