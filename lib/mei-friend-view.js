...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      if (!range) { //ext !== ".mei") {
        this.verovioPanel.innerHTML =
          "<h2>Not an MEI encoding. Notation will only be rendered for files containing MEI markup.</h2>";
        this.hideByID(this.controlsForm.id);
        return;
      }
      ...
      this.waitForElement(`#${this.verovioBtn.id}`, () => {
        $(`#${this.verovioBtn.id}`).off('click').on('click', (e) => {
          ...
          item.setText(this.vrvToolkit.getMEI(opt));
          this.setFocusToVerovioPane()
        });
      });
  ...
  updateAll(textEditor, options = {}, setCursorToPageBeginning = false) {
    ...
    console.info('updateAll: tabLength: ' +
      textEditor.getTabLength() + ', editor: ', textEditor);
  ...
  redoVerovioLayout() {
    ...
    if (!this.speedMode) this.pageCount = this.vrvToolkit.getPageCount();
  }
  ...
  showCurrentPage(page = this.currentPage) {
    console.info('showCurrentPage(): ' + page +
      ', pageCount: ' + this.pageCount);
    if (this.pageCount === 0) return; // no data loaded
    if (!this.validateCurrentPage()) page = 1;
    try {
      var svg = this.vrvToolkit.renderToSVG(page, {})
      this.verovioPanel.innerHTML = svg;
    } catch (e) {
      console.log(e);
      return;
    }
  ...
  setCursorToPageBeginning(textEditor) {
    ...
    if (id == '') {
      ...
      if (rg) {
        ...
        if (m) {
          id = domutils.getFirstInMeasure(m,
            domutils.navigationElementsSelector, stNo, lyNo);
        }
      }
    }
    rg = utils.locateIdInBuffer(buffer, id);
    if (rg) {
      textEditor.setCursorBufferPosition([rg.start.row, rg.start.column]);
    }
  ...
  handleClickOnNotation(e, textEditor) {
    ...
    if (el.getAttribute('class') == 'note')
      console.info('CLICK note: x ', domutils.getX(el));
    else if (el.getAttribute('class') == 'chord')
      console.info('CLICK chord: x ', domutils.getX(el));
    let range;
  ...
  updateHighlight(textEditor) {
    ...
    else ids.push(utils.getElementIdAtCursor(textEditor.getBuffer(),
      textEditor.getCursorBufferPosition()));
    for (id of ids) {
      id = utils.getElementIdAtCursor(textEditor.getBuffer(),
        textEditor.getCursorBufferPosition());
      console.info('updateHlt new to id: ', id);
      if (id) {
  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    let id = this.lastNoteId;
    if (id == '') { // empty note id
      this.setCursorToPageBeginning(textEditor); // re-defines lastNotId
      id = this.lastNoteId;
    };
    let elementsList = '.note, .rest, .mRest, .beatRpt, .halfmRpt, .mRpt';
    let buffer = textEditor.getBuffer();
    ...
    console.info('Navigate ' + direction + ' ' +
      elementName + '-wise for: ', element);
    ...
    if (!measure) {
      ...
      if (elementName == 'note' || elementName == 'measure') {
        id = domutils.getIdOfNextSvgElement(
          element, direction, elementsList, elementName);
        // when no id on screen, turn page
        if (!id && direction == 'forwards') {
          this.updatePage(textEditor, 'forwards');
          this.setFocusToVerovioPane();
          id = domutils.getFirstInMeasure(document.querySelector('.measure'),
            elementsList, stNo, lyNo);
        } else if (!id && direction == 'backwards' && this.currentPage > 1) {
          this.updatePage(textEditor, 'backwards');
          this.setFocusToVerovioPane();
          let m = document.querySelectorAll('.measure'); // all measures of page
          if (m) {
            id = domutils.getLastInMeasure(m[m.length - 1], // use last one
              elementsList, stNo, lyNo)
          }
        }
      }
      ...
      if (elementName == 'layer') {
        let x = domutils.getX(element);
        console.info('navigate(layer): x: ' + x + ', element: ', element);
        let layers = Array.from(measure.querySelectorAll('.layer'));
        if (direction == 'upwards') layers.reverse();
        console.info('layers: ', layers);
        let found = false;
        let diff = Number.MAX_VALUE;
        for (l of layers) { // go thru all layers until current layer id found
          console.info('l: ', l);
          if (found) {
            let newEls = l.querySelectorAll(elementsList);
            // console.info('newEls: ', newEls);
            for (e of newEls) {
              let newDiff = Math.abs(x - domutils.getX(e));
              // console.info('e: ', e);
              // console.info('x: ' + x + ', diff: ' + newDiff);
              if (newDiff <= diff) {
                diff = newDiff;
                id = e.getAttribute('id');
              }
              if (diff > newDiff || newDiff == 0) { // stopp condition
                found = false;
                break;
              }
            }
          }
          if (l.getAttribute('id') == layer.getAttribute('id')) found = true;
        }
      } // up/down in layers
  ...
  delete(textEditor, id) {
    ...
    if (att.modelControlEvents.concat(['accid', 'artic', 'clef', 'octave'])
      ...
      if (element.nodeName == 'octave') { // reset notes inside octave range
        ...
        speed.findAndModifyOctaveElements(textEditor, this.xmlDoc, id1, id2,
          dis_place, dis, false);
      ...
      let p1 = textEditor.getCursorBufferPosition();
      let first = true;
      let childList = element.childNodes;
      for (let i = 0; i < childList.length; i++) {
        if (childList[i].nodeType === Node.TEXT_NODE) continue;
        if (first) {
          speed.replaceInTextEditor(textEditor, element, false, childList[i]);
          first = false;
        } else {
          textEditor.insertNewline();
          textEditor.insertText(speed.xmlToString(childList[i]));
        }
        selectedElements.push(childList[i].getAttribute('xml:id'))
        element.parentNode.insertBefore(childList[i--], element);
      }
      let p2 = textEditor.getCursorBufferPosition();
      textEditor.setSelectedBufferRange([p1, p2]);
      textEditor.autoIndentSelectedRows();
      textEditor.setCursorBufferPosition(p2);
    } else {
    ...
    this.speedMode ? this.updateLayout(textEditor) : this.updateData(textEditor);
    this.setFocusToVerovioPane();
  }
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '',
    ...
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      // use Verovio insert action for implemented elements in Verovio 3.2
      if (false) { //elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // || elementName == 'dir' || elementName == 'dynam') {
        editorAction = {
          action: 'chain',
          param: [{
            action: 'insert',
        ...
        if (endId) { // check elementName for required and optional endId!!
          editorAction.param[0].param.endid = endId;
        }
        console.info('editorAction: ', editorAction);
        // add text
        // if (formString != '' && (elementName == 'dir' || elementName == 'dynam' ||
        //     elementName == 'tempo' || elementName == 'fing' || elementName == 'reh')) {
        //   editorAction.param.text = formString;
        // }
        // if (useTstamps) {
        //   editorAction.param.useTstamps = true;
        // }
        // try {
        //   editInfo = this.vrvToolkit.editInfo();
        //   uuid = editInfo.uuid;
        // } catch (exception) {
        //   console.error(exception);
        // }
        // console.log('vrvToolkit.editInfo(): ', editInfo);

        // add form attribute
        if (formString && (elementName == 'hairpin' || elementName == 'fermata' ||
            elementName == 'mordent' || elementName == 'trill' || elementName == 'turn')) {
          editorAction.param.push({
            action: 'set',
            param: {
          ...
          // this.executeSetAction(uuid, 'form', formString);
        }
        // insert direction for pedal (and a default vgrp, even not yet MEI standard)
        // if (elementName == 'pedal') {
        //   this.executeSetAction(uuid, 'dir', placementString);
        //   this.executeSetAction(uuid, 'vgrp', '100');
        //   placementString = '';
        // }
        // placement or curvedir
        if (placementString) {
          if (att.attCurvature.includes(elementName)) {
            attribute = 'curvedir';
          } else if (att.attPlacement.includes(elementName)) {
            attribute = 'place';
          }
          if (attribute) {
            editorAction.param.push({
        ...
        // if (elementName == 'trill' && endId) { // set extender attribute, if two notes selected for a trill
        //   this.executeSetAction(uuid, 'extender', 'true');
        // }
        editorAction.param.push({
          action: 'commit'
        });
        ...
        console.log('vrvToolkit.getLog(): \n', this.vrvToolkit.getLog() + '; res: ', res);

        this.updateNotation = false; // dont update notation when text is inserted
        ...
        // this.updateHighlight(textEditor);
        this.setFocusToVerovioPane();
      } else { // insert new code into MEI file
  ...
  addBeamElement(textEditor, elementName = 'beam') {
    this.loadXml(textEditor);
    this.selectedElements = speed.filterElements(this.selectedElements, this.xmlDoc);
    this.selectedElements =
      utils.sortElementsByScoreTime(this.selectedElements, this.vrvToolkit);
    ...
    if (par1.getAttribute('xml:id') == n2.parentNode.getAttribute('xml:id')) {
      ...
      for (let i = 0; i < nodeList.length; i++) {
        ...
        if (nodeList[i].getAttribute('xml:id') == id2) {
          ...
          speed.replaceInTextEditor(textEditor, n, true, beam);
          textEditor.autoIndentSelectedRows();
          break;
        }
        if (insert) {
  ...
  addOctaveElement(textEditor, dis_place = 'above', dis = '8') {
    this.loadXml(textEditor);
    if (this.selectedElements.length < 1) return;
    let buffer = textEditor.getBuffer();
    ...
    // add it to the textEditor
    this.updateNotation = false;
    let checkPoint = buffer.createCheckpoint();
    let range = utils.locateIdInBuffer(buffer, id1);
    if (range) {
      let begin = utils.moveCursorToEndOfMeasure(textEditor, range.start); // resets selectedElements!!
      textEditor.insertText(speed.xmlToString(octave));
      textEditor.insertNewline();
      textEditor.setSelectedBufferRange([begin, [begin[0] + 2, begin[1]]]);
      textEditor.autoIndentSelectedRows();
      textEditor.setCursorBufferPosition(begin);
    }
  ...
  invertPlacement(textEditor, modifier = false) {
    ...
    for (id of ids) {
      ...
      if (false && att.attPlacement.includes(elementName) && modifier) { // CTRL + X @place="between/within"
        ...
        attribute = 'num.place';
        value = 'above';
        if (attr.hasOwnProperty(attribute) && attr[attribute] == value) {
          value = 'below';
        }
        this.executeSetAction(id, attribute, value);
  ...
  invertPlacementDom(textEditor, modifier = false) {
    ...
    for (id of ids) {
      ...
      if (att.attPlacement.includes(el.nodeName)) {
        ...
        range = speed.replaceInTextEditor(textEditor, el, true);
        textEditor.autoIndentSelectedRows();
        ...
        for (note of noteList) {
          ...
          this.updateNotation = false; // no need to reload xmlDOM
          range = speed.replaceInTextEditor(textEditor, note, true);
          textEditor.autoIndentSelectedRows();
        }
      } else {
      ...
      if (range) textEditor.setCursorBufferPosition(range.end);
      this.selectedElements = ids;
      this.updateLayout(textEditor);
      this.setFocusToVerovioPane();
    }
