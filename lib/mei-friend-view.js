  ...
  */
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '', formString = '') {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      //  validate selected elements to be notes, chords
      ...
      if (this.selectedElements.length == 1 && (elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin' || elementName == 'gliss')) {
        ...
        endId = this.getIdOfNextElement(buffer, range.start.row, 'note')[0];
  ...
  forcePagination(data) {
    ...
    if (count > 100 || true) { // deactivate pagination permanently
  ...
  createHTMLElements(scale) {
    ...
    atom.tooltips.add(this.zoomCtrls, {
    ...
    // breaksLabel.innerText = 'B: ';
    // atom.tooltips.add(breaksLabel, {
    //   title: 'Define breaks behavior of notation'
    // });
    ...
    // TODO: read breaks options from vrvToolkit automatically
    //console.info('tkOptions: ' + JSON.stringify(this.tkOptions.groups.1-general)); //['1-general'][0]['options'][0]['breaks'][0]));
    ...
    atom.tooltips.add(this.navigateCtrls, {
      title: 'Navigate within notation'
    });
