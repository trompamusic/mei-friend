...
export default class MeiFriendView {
  ...
  constructor(vt) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      let range; // find <mei in buffer of textEditor to identify an MEI file
      item.getBuffer().scan(/(?:<mei)/, (obj) => {
        range = obj.range;
        obj.stop();
      });
  ...
  setCursorToPageBeginning(textEditor) {
    ...
    let range = locateIdInBuffer(textEditor.getBuffer(), id);
    if (range) {
  ...
  handleClickOnNotation(e, textEditor) {
    ...
    if (((navigator.appVersion.indexOf("Mac") !== -1) && e.metaKey) || e.ctrlKey) {
      ...
      range = locateIdInBuffer(textEditor.getBuffer(), itemId);
      if (range) {
        textEditor.setCursorBufferPosition([range.start.row, range.start.column]);
      }
      this.selectedElements = [];
    ...
    let startid = getAttributeById(textEditor.getBuffer(), itemId);
    ...
    if (!startid) { // work around for tstamp/staff
      ...
      // startid = getAttributeById(textEditor.getBuffer(), itemId, attribute = 'tstamp');
      // console.info('staff: ', startid);
      if (!range) range = locateIdInBuffer(textEditor.getBuffer(), itemId);
      if (range)
        textEditor.setCursorBufferPosition([range.start.row, range.start.column]);
    }
  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    if (typeof $(measure).attr('id') === 'undefined') {
      ...
      let range = locateIdInBuffer(buffer, id);
      let row = 0;
      if (range) row = range.start.row;
      let startLayerN = parseInt(getElementAttributeAbove(buffer, row, 'layer', numberLikeString)[0]);
      let startStaffN = parseInt(getElementAttributeAbove(buffer, row, 'staff', numberLikeString)[0]);
    ...
    if (range = locateIdInBuffer(buffer, id)) {
      textEditor.setCursorBufferPosition([range.start.row, range.start.column]);
    }
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '', formString = '', useTstamps = false) {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      let range = locateIdInBuffer(buffer, startId);
      startElementName = buffer.lineForRow(range.start.row).match(/(?:<)([a-zA-Z]+\b)/)[1];
      if (startElementName != 'note' && startElementName != 'chord' && startElementName != 'mRest' && startElementName != 'multiRest') {
      ...
      if (endId) {
        range = locateIdInBuffer(buffer, endId);
        endElementName = buffer.lineForRow(range.start.row).match(/(?:<)([a-z]+\b)/)[1];
        if (endElementName != 'note' && endElementName != 'chord' && endElementName != 'rest') {
      ...
      if (elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // || elementName == 'dir' || elementName == 'dynam') {
        editorAction = {
          action: 'chain',
          param: [{
            action: 'insert',
        ...
        if (placementString) {
          if (attCurvature.includes(elementName)) {
            attribute = 'curvedir';
          } else if (attPlacement.includes(elementName)) {
            attribute = 'place';
          }
          if (attribute) {
            editorAction.param.push({
      ...
      range = locateIdInBuffer(buffer, uuid);
      if (range) {
        if (elementName == 'dir' || elementName == 'dynam' || elementName == 'tempo') {
  ...
  invertPlacement(textEditor, modifier = false) {
    ...
    for (id of this.selectedElements) {
      ...
      range = locateIdInBuffer(buffer, id);
      if (range) {
      ...
      let attribute = '';
      let value = 'above';
      if (false && attPlacement.includes(elementName) && modifier) { // CTRL + X @place="between/within"
        console.info('invertPlacement CTRL + X');
        attribute = 'place';
        if (attr.hasOwnProperty(attribute) && (dataPlacement.includes(value) && attr.place != 'between')) {
          value = 'between';
        }
        ...
        if (attr.hasOwnProperty('startid')) {
          ...
          startIdRange = locateIdInBuffer(buffer, startId);
          console.info('startId: ' + startId + ', startIdRange: ', startIdRange);
          staffProps = getElementAttributeAbove(buffer, startIdRange.start.row, 'staff'); // find staff number and row of startid
          staffNAbove = getElementAttributeAbove(buffer, staffProps[1], 'staff')[0]; // find staff number above startid
          staffNbelow = getElementAttributeBelow(buffer, startIdRange.start.row, 'staff')[0]; // find staff number below startid
        ...
        // measure = getElementAttributeAbove(buffer, range.start.row, 'measure', xmlIdString);
        this.executeSetAction(id, 'staff', value);
      } else if (attPlacement.includes(elementName)) { // att.Placement for dir, dynam, trill, fermata, ... @place="below/above"
        attribute = 'place';
        if (attr.hasOwnProperty(attribute) && (dataPlacement.includes(value) && attr.place != 'below')) {
          value = 'below';
        }
        this.executeSetAction(id, attribute, value);
        // if (attr.hasOwnProperty('staff')) { // if attribute staff is present, set it to match those of elementName
        //   staffProps = getElementAttributeAbove(buffer, range.start.row, 'staff')[0]; // find staff number and row
        //   this.executeSetAction(id, 'staff', staffProps);
        // }
      } else if (attCurvature.includes(elementName)) { // att.Curvature for slur, tie, ... @curvedir="above/below"
        attribute = 'curvedir';
        if (attr.hasOwnProperty(attribute) && attr.curvedir == 'above') {
          value = 'below';
        }
        this.executeSetAction(id, attribute, value);
      } else if (attStems.includes(elementName)) { // att.Stems for note, chord @stem.dir="up/down"
        attribute = 'stem.dir';
        value = 'up';
