...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      // zoom in/out quickly with CMD/CTRL + wheel
      this.element.addEventListener('wheel', event => {
        if (((navigator.appVersion.indexOf("Mac") !== -1) && event.metaKey) ||
          event.ctrlKey) {
          this.zoom(item, Math.sign(event.deltaY) * 5);
        }
      });
  ...
  redoVerovioLayout() {
    ...
    if (!this.speedMode) {
      ...
      if (this.lastNoteId)
        this.currentPage = this.vrvToolkit.getPageWithElement(this.lastNoteId);
  ...
  updateZoomSliderTooltip() {
    let tt = atom.tooltips.findTooltips(this.zoomCtrl);
    let toolTipText = 'Notation scale: ' + this.zoomCtrl.value + "%";
    // console.info('Zoomctr.TT: ', tt);
    if (tt) {
      tt[0].options.title = toolTipText;
    } else {
    ...
    this.toolTipTimeOutHandle = setTimeout(() => tt[0].hide(), 1500);
  }
  ...
  */
  ...
  executeSetAction(id, attribute, value) {
    ...
    try {
      ...
      console.error('executeSetAction() editorAction error: ', exception);
    }
    try {
      ...
      console.error('executeSetAction() commit error: ', exception);
    }
  }
  ...
  createHTMLElements(scale) {
    ...
    this.zoomCtrl.setAttribute('min', 10);
