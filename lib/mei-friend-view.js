  ...
  */
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '', formString = '') {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      if (elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // use Verovio insert action for slur and hairpin
        editorAction = {
          action: 'insert',
          param: {
        ...
        if (elementName == 'hairpin') {
          this.executeSetAction(uiid, 'form', formString);
        }
        if (placementString != '') {
        ...
        uuid = editInfo.uuid;
        // TODO find id of new element in mei
        this.updateAll(textEditor);
        this.setFocusToVerovioPane();
