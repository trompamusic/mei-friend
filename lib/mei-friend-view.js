...
export default class MeiFriendView {
  ...
  constructor(tk) {
    ...
    this.vodMode = false; // Verovio on Diet mode (just feeds on page to Verovio to reduce drawing time)
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      this.waitForElement(`#${this.breaksSelector.id}`, () => {
        $(`#${this.breaksSelector.id}`).off('change').on('change', () => {
          ...
          this.vodMode = (this.vrvOptions.breaks == 'line' ||
            this.vrvOptions.breaks == 'encoded') ? true : false;
          this.encodingHasChanged = true;
          console.info('breaksSelector: vodMode: ' + this.vodMode);
          this.updateAll(item);
          this.setFocusToVerovioPane()
        });
      });
  ...
  updateAll(textEditor, options = {}, setCursorToPageBeginning = false) {
    ...
    if (this.vodMode) {
      let mei = this.loadXml(textEditor);
  ...
  loadXml(textEditor) {
    ...
    if (this.encodingHasChanged) {
      ...
      // vod.getBreaksFromToolkit(this.vrvToolkit, textEditor.getBuffer().getText());
    }
    let mei = vod.getVodPageFromDom(this.xmlDoc,
      this.currentPage, this.whichBreaks);
    // console.info('loadXml mei: ', mei);
    return mei;
  }
  ...
  redoVerovioLayout() {
    ...
    if (!this.vodMode) this.pageCount = this.vrvToolkit.getPageCount();
  }
  ...
  handleClickOnNotation(e, textEditor) {
    ...
    chordId = utils.insideChord(itemId);
  ...
  */
  navigate(textEditor, elementName = 'note', direction = 'forwards') {
    ...
    if (typeof $(measure).attr('id') === 'undefined') {
      ...
      if (elementName == 'note') {
        ...
        do {
          tmp = utils.getIdOfNextElement(buffer, row, 'noteOrRest', direction);
          if (tmp[0] === '') break;
          id = tmp[0];
          row = parseInt(tmp[1]);
  ...
  addControlElement(textEditor, elementName = 'slur', placementString = '',
    ...
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      if (false) { //elementName == 'slur' || elementName == 'tie' || elementName == 'hairpin') { // || elementName == 'dir' || elementName == 'dynam') {
        ...
        utils.moveCursorToEndOfMeasure(textEditor); // !! resets selectedElements !!
        utils.insertTabs(textEditor, 1);
  ...
  addControlElementDom(textEditor, elementName, placementString, formString, useTstamps) {
    ...
    console.info('addControlElementDom(): ', startElement, endElement);
    // create element to be inserted
    let newElement = this.xmlDoc.createElementNS(vod.nameSpace, elementName);
    let uuid = elementName + '-' + utils.generateUUID();
    newElement.setAttribute('xml:id', uuid);
    ...
    console.info('addControlElementDom() newElement: ', newElement);
    // console.info('addControlElementDom() range: ', range);
    if (range) {
      this.updateNotation = false;
      range.start = utils.moveCursorToEndOfMeasure(textEditor, range.start); // resets selectedElements!!
      utils.insertTabs(textEditor, 2);
      textEditor.insertText(vod.xmlToString(newElement));
      textEditor.insertNewline();
    ...
    var measureId = vod.getParentNode(startElement, 'measure').getAttribute('xml:id');
    console.info('addControlElementDom() measure: ', measureId);
    m = this.xmlDoc.querySelector("[*|id='" + measureId + "']");
    this.xmlDoc.querySelector("[*|id='" + measureId + "']").appendChild(newElement);
    console.info('addControlElementDom() measure: ', m);
    console.info('QUERY: ', this.xmlDoc.querySelector("[*|id='" + uuid + "']"));
  ...
  invertPlacement(textEditor, modifier = false) {
    ...
    let elementName = '';
    for (id of this.selectedElements) {
      let chordId = utils.insideChord(id);
    ...
    for (id of this.selectedElements) {
      ...
      if (att.attPlacement.includes(el.nodeName)) {
        ...
        for (note of noteList) {
          ...
          this.updateNotation = false; // no need to reload xmlDOM
          vod.replaceInBuffer(buffer, note);
        }
      } else {
        console.info('invertPlacementDom(): ' + el.nodeName + ' contains no elements to invert.');
      }
      // SLOW To be replaced by more efficient code...
      // textEditor.setText(new XMLSerializer().serializeToString(this.xmlDoc));
      //
  ...
  updateNotationToTextposition(textEditor, update = true) {
    var page;
    if (this.vodMode) {
      page = vod.getPageNumberAtCursor(textEditor, this.whichBreaks);
    } else {
