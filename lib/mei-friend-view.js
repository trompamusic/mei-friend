...
export default class MeiFriendView {
  ...
  */
  ...
  toggleArtic(textEditor, artic = "stacc") {
    this.loadXml(textEditor);
    for (let noteId of this.selectedElements) {
      let note = this.xmlDoc.querySelector("[*|id='" + noteId + "']");
      if (['note', 'chord'].includes(note.nodeName)) {
        console.info('note: ', note);
        note = utils.attrAsElements(note);
        console.info('elementAfter: ', );
        let articChildren;
        let add = false;
        if (note.hasChildNodes() && (articChildren = note.querySelectorAll('artic'))) {
        ...
        if (add) {
          ...
          articElement.setAttribute('xml:id', 'artic-' + utils.generateUUID());
          articElement.setAttribute('artic', artic);
          note.appendChild(articElement);
        }
        speed.replaceInBuffer(textEditor.getBuffer(), note);
      }
    }
  }
