...
export default class MeiFriendView {
  ...
  */
  ...
  addTie(textEditor) {
    this.addSlur(textEditor, elementName = 'tie');
  }
  ...
  addSlur(textEditor, elementName = 'slur') {
    if (this.selectedElements.length <= 2 && this.selectedElements.length > 0) {
      ...
      if (this.selectedElements.length == 1) {
        ...
        endId = this.selectedElements[1];
      }
      slurString = '<' + elementName + ' xml:id="' + uuid + '" startid="#' + startId + '" endid="#' + endId + '" />';
      pos = this.moveCursorToEndOfMeasure(textEditor); // !! resets selectedElements !!
      this.insertTabs(textEditor, 1);
      textEditor.insertText(slurString);
      ...
      console.info('addSlur() added: "' + slurString + '"');
      this.lastNoteId = startId; // start moving cursor from starting note
    } else {
      console.info('addSlur() nothing added. ' + this.selectedElements.length + ' selected elements.');
      return null;
    }
  }
