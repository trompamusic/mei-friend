...
export default class MeiFriendView {
  ...
  supplyElement(txtEdr)  {

    this.loadXml(txtEdr);
    let ids = speed.filterElements(this.selectedElements, this.xmlDoc);
  
    console.log("Run supplyElement() on " + ids)
    this.updateNotation = false;
    let i, range;
    for (i = 0; i < ids.length; i++) {
      let id = ids[i];
      ...
      var temp_id = 'supplied_' + utils.generateUUID();
      sup.setAttributeNS(speed.xmlNameSpace, 'xml:id', temp_id);
      parent.replaceChild(sup, el);
      sup.appendChild(el);
  
      uuid ? ids[i] = uuid : ids[i] = id;
      range = speed.replaceInTextEditor(txtEdr, parent, true);
      txtEdr.autoIndentSelectedRows();
    }
  ...
  supplyAttribute(txtEdr, attrName)  {
    console.log("supply " + attrName + "!");
    this.loadXml(txtEdr);
    let ids = speed.filterElements(this.selectedElements, this.xmlDoc);
  
    console.log("Run supplyAttribute(" + attrName + ") on " + ids)
    this.updateNotation = false;
    let i, range;
    for (i = 0; i < ids.length; i++) {
      let id = ids[i];
      let el = this.xmlDoc.querySelector("[*|id='" + id + "']");
      if (!(el | el.hasAttribute(attrName))) { console.log("No " + attrName + "!"); continue; }
      let uuid;
  
      // create <supplied> with @xml:id
      let sup = document.createElementNS(speed.meiNameSpace, 'supplied');
      var temp_id = 'supplied_' + utils.generateUUID();
      sup.setAttributeNS(speed.xmlNameSpace, 'xml:id', temp_id);
  
      // create <artic> with @xml:id and @artic
      let attr2el = document.createElementNS(speed.meiNameSpace, attrName);
      temp_id = attrName + '_' + utils.generateUUID();
      attr2el.setAttributeNS(speed.xmlNameSpace, 'xml:id', temp_id);
      let supplied_attr_value = el.attributes[attrName].value;
      attr2el.setAttribute(attrName, supplied_attr_value);
  
  
      sup.appendChild(attr2el);
      el.appendChild(sup);
  ...
  pianoDynamicsBetween(txtEdr)  {
    console.log("Run pianoDynamicsBetween()");
    this.loadXml(txtEdr);
    let ids = speed.filterElements(this.selectedElements, this.xmlDoc);
    ...
    for (i = 0; i < ids.length; i++) {
      let id = ids[i];
      let el = this.xmlDoc.querySelector("[*|id='" + id + "']");
      if (!(el)) { console.log("No element selected!"); continue; }
      let uuid;

      el.setAttribute('place', 'between');
      el.setAttribute('staff', '1 2');
