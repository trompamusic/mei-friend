...
export default {
  ...
  activate(state) {
    ...
    var worker = new Worker(path.resolve(__dirname, 'speed-worker.js'));
  ...
  activateSubscriptions(vrvToolkit) {
    ...
    this.worker = new Worker(path.resolve(__dirname, 'speed-worker.js')); //, {      type: "module"    });
