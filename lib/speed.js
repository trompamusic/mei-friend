...
function readSection(xmlScore, pageNo, spdScore, whichBreaks = ['sb', 'pb']) {
  ...
  return function digDeeper(section) {
    ...
    for (let i = 0; i < lgt; i++) {
      ...
      if (currentNodeName == 'scoreDef' && p < pageNo) {
        ...
        if (children[i].hasAttribute('key.sig')) {
          ...
          for (let staff of staffDefs) {
            let k = staff.querySelector('keySig');
            if (k) {
        ...
        if (children[i].hasAttribute('meter.count') && children[i].hasAttribute('meter.count')) {
          ...
          let staffDefs = spdScore.querySelectorAll('staffDef');
          for (let staff of staffDefs) {
            var k = staff.querySelector('meterSig');
            if (k) {
              k.setAttribute('count', meterCountValue);
              k.setAttribute('unit', meterUnitValue);
            } else {
              staff.appendChild(meterSigElement.cloneNode(true));
            }
          }
        }
      }
    ...
    // console.info('POST LOOP MATCHING: ');
    // let e, s;
    // for (e = 0; e < endingElements.length; e++) {
    //   for (s = 0; s < startingElements.length; s++) {
    //     if (endingElements[e] == startingElements[s]) {
    //       console.info('elements removed:', endingElements[e]);
    //       console.info('and: ', startingElements[s]);
    //       startingElements.splice(s--, 1)
    //       endingElements.splice(e--, 1);
    //       break;
    //     }
    //   }
    // }
