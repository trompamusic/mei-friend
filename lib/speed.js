    ...
    for (let i = 0; i < lgt; i++) {
      ...
      if (clefList && clefList.length > 0 && p < pageNo) {
        ...
        for (let clef of clefList) { // check clefs of measure, ignore @sameas
          ...
          let stffNo = clef.closest('staff, staffDef').getAttribute('n');
...
export function replaceInTextEditor(textEditor, xmlNode, select = false) {
  ...
  if (noReplaced < 1)
    ...
    let range = getRangeOfString(textBuffer, newMEI);
    if (range) {
