...
export function getAttributeById(buffer, itemId, attribute = 'startid') {
  var searchElementName = '<[\\w.-]+?\\s+?(?:xml:id="' + itemId + '")[^>]*?>';
  let range = locateIdInBuffer(buffer, itemId, searchElementName);
  var elementString = '';
  if (range) elementString = buffer.getTextInRange(range);
  // console.info('elementString: |' + elementString + '|');
  searchElementName = `(?:` + attribute + `=)(?:['"]#)(\\S+?)(?:['"])`;
  let startid = elementString.match(new RegExp(searchElementName));
...
// returns @startid of element with id, if present, null otherwise
export function takeStartid(textEditor, id) {
