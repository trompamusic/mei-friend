...
export function renumberMeasures(xmlDoc, textEditor, startNumber = 1) {
  let buffer = textEditor.getBuffer();
  ...
  for (i = 0; i < lgt; i++) {
    ...
    // speed.replaceInBuffer(buffer, measureList[i]);
    console.info(measureList[i].getAttribute('n') + ' to be changed to ' + n + ', right:' + right + ', metcons:' + metcons);
