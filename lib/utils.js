...
export function getIdOfNextSvgElement(currentElement,
  elementNames = ['note, rest, mRest, beatRpt, halfmRpt, mRpt'],
  direction = 'forwards') {
  let container = $('div#verovio-panel'); // overarching element
  console.info("getIdOfNextSvgElement: container ", container);
  let startLayerN = currentElement.parents('layer').attr('data-n');
  let startStaffN = currentElement.parents('staff').attr('data-n');
  elementList = container.find(elementNames);
  console.info("getIdOfNextSvgElement: elementList ", elementList);
  n = elementList.find(currentElement);
  console.info("n: " +  n + ", elementList(i/i+1): " + elementList[n] + elementList[n+1]);
  let id = "";
  if (direction == 'forwards') {
    for (let i = n + 1; i < elementList.length; i++) {
      if ((elementList[i].parent('layer').attr('data-n') == startLayerN) &&
        (elementList[i].parent('staff').attr('data-n') == startStaffN))
        id = elementList[i].attr('id');
      break;
    }
  } else if (direction == 'backwards') {
    for (let i = n - 1; i > 0; i--) {
...
export function sortElementsByScoreTime(arr, tk) {
  ...
  for (j = arr.length; j > 1; --j) {
    for (i = 0; i < (j - 1); ++i) {
      let x = tk.getTimeForElement(arr[i]);
      if (x <= 0) {
        console.info('Utils.sortElementsByScoreTime(): score time zero: ' + arr[i]);
        continue;
      }
      let y = tk.getTimeForElement(arr[i + 1]);
      if (y <= 0) {
        console.info('Utils.sortElementsByScoreTime(): score time zero: ' + arr[i + 1]);
        continue;
      }
      if (x > y) { // swap elements
