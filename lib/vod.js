...
function readSection(pageNo, vodScore, breaksList = ['sb', 'pb']) {
  ...
  return function digDeeper(section) {
    ...
    for (let i = 0; i < lgt; i++) {
      ...
      // update scoreDef
      var scoreDefList = children[i].querySelectorAll('scoreDef, staffDef');
      if (scoreDefList && scoreDefList.length > 0) {
        console.info('scoreDefList: ', scoreDefList);
      }
      var clefList = children[i].querySelectorAll('clef');
      if (clefList && clefList.length > 0 && p < pageNo) {
        console.info('clefList: ', clefList);
        var clef = clefList.item(clefList.length - 1); // last clef
        ...
        // console.info('querySelectorAll: ', vodScore.querySelectorAll("staffDef[n="+ staffNumber + "]"));
      ...
      if (['ending'].includes(currentNodeName) && (children[i].querySelector(breaksListSelector))) {
