...
export function getVodPage(xmlDoc, systemNo = 1) {
  ...
  for (let i = 0; i < elements.length; i++) {
    if (elements.item(i).nodeName == 'sb') {
      sb++;
      continue;
    }
    if (elements.item(i).nodeName == 'pb') {
      pb++;
      continue;
    }
...
export function dummyMeasure(xmlNode, staves = 2) {
  ...
  note1 = xmlNode.createElement('note');
  note1.setAttribute('pname', 'a');
  note1.setAttribute('oct', '3');
  note1.setAttribute('dur', '1');
  layer1 = xmlNode.createElement('layer')
  layer1.setAttribute('n', '1');
  layer1.appendChild(note1);
  staff1 = xmlNode.createElement('staff');
  staff1.setAttribute('n', '1');
  staff1.appendChild(layer1);
  m.appendChild(staff1);
  staff2 = xmlNode.createElement('staff');
  staff2.setAttribute('n', '2');
  layer2 = xmlNode.createElement('layer')
  layer2.setAttribute('n', '1');
  layer2.appendChild(xmlNode.createElementNS('http://www.music-encoding.org/ns/mei', 'mRest'));
  staff2.appendChild(layer2);
  m.appendChild(staff2);
  return m;
}
