...
export function replaceInBuffer(textBuffer, xmlNode) {
  var itemId = xmlNode.getAttribute('xml:id');
  var searchSelfClosing = '(?:<' + xmlNode.nodeName + ')\\s+?(?:xml:id="' +
    itemId + '")(.*?)(?:/>)';
  let range;
  ...
  // textBuffer.replace(/(?:<${xmlNode.nodeName})/, 'TREFFER');
}
